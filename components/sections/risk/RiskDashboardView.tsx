...
export function RiskDashboardView() {
  ...
  // Sin GPS real → pedir permiso
  if (!isLoading && lat === null) {
    return (
      <div className="flex flex-col items-center justify-center gap-6 py-16 text-center">
        <div className="text-5xl">📍</div>
        <div className="space-y-2">
          <h3 className="text-lg font-bold text-text-primary">Necesitamos tu ubicación</h3>
          <p className="text-sm text-text-muted max-w-xs">
            YakuAlert calcula el riesgo real de huaycos según tu distancia a quebradas activas. Sin GPS no hay cálculo.
          </p>
        </div>
        <button
          onClick={() => window.location.reload()}
          className="px-5 py-2.5 bg-primary text-white text-sm font-bold rounded-xl hover:bg-blue-600 transition-all"
        >
          Activar GPS y calcular riesgo
        </button>
      </div>
    )
  }
  ...
  const quickStats = [
    {
      ...
      icon: '💧',
      label: 'Lluvia 48h',
      value: lluviaMm !== null ? lluviaMm.toFixed(1) : '0.0',
      unit: 'mm',
    ...
    },
    {
      id: 'senamhi',
      icon: '📡',
      label: 'Alerta SENAMHI',
      value: desglose?.vulnerabilidad && desglose.vulnerabilidad > 0 ? 'ACTIVA' : 'Normal',
      unit: 'en vivo',
      status: (desglose?.vulnerabilidad && desglose.vulnerabilidad > 0 ? 'red' : 'slate') as 'red' | 'amber' | 'slate',
    },
    {
      id: 'lastAlert',
      icon: '🎯',
      label: 'Nivel de Riesgo',
      value: score && score > 66 ? 'ALTO' : score && score > 33 ? 'MEDIO' : 'BAJO',
      unit: 'calculado',
  ...
  return (
    <div className="w-full space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2.5 border-t border-border/30 pt-2.5">
              ...
              <div>
                ...
                <span className="text-[11px] block mt-0.5 leading-normal">
                  Pronóstico satelital Open-Meteo. Lluvias de 5mm o menos dan 0 pts. Escala lineal hasta lluvias torrenciales de 60mm o más (40 pts).
                </span>
              </div>
            </div>
            <div className="flex gap-2.5 border-t border-border/30 pt-2.5">
              ...
              <div>
                ...
                <span className="text-[11px] block mt-0.5 leading-normal">
                  Alertas vigentes emitidas por SENAMHI vía GeoServer WFS. Si hay alerta activa Naranja o Roja en tu distrito, se añaden 20 pts.
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        {/* Columna Izquierda: Gauge + Banner */}
        ...
        {/* Columna Derecha: Stats + Enlace Mapa */}
        <div className="space-y-6 w-full">
      ...
      {/* Plan de Evacuación a ancho completo */}
