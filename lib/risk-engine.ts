...
function distanciaAlCauce(lat: number, lon: number, cauce: DangerZone): number {
...
function puntajeCaruce(lat: number, lon: number, cauces: DangerZone[]): {
  pts: number
  cauceCercano: string | null
  distanciaMetros: number | null
} {
...
function puntajeLluvia(lluviaMm: number): number {
  const LLUVIA_MIN = 5
  const LLUVIA_MAX = 60
...
function puntajeVulnerabilidad(alertaActiva: boolean): number {
  return alertaActiva ? 20 : 0
}

function nivelDesdeScore(score: number): 'BAJO' | 'MODERADO' | 'ALTO' {
...
export function calcularRiesgo(input: RiskInput): RiskResult {
  ...
  const { pts: cauce, cauceCercano, distanciaMetros } = puntajeCaruce(lat, lon, cauces)
